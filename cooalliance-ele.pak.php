...
function coo_alliance_ele_scripts()
{
    ...
    wp_enqueue_script('coo-infinite-scroll', COOELEMENTOR_ASSETS . '/js/podcast-pagination.js', array('jquery'), null, true);

    wp_localize_script('coo-infinite-scroll', 'cooalliance_ajax', array(
...
function render_coo_podcast_captivate_meta_data($post)
{
    ...
    <input type="text" id="podcast_player_link" name="podcast_player_link"
        value="<?php echo esc_attr($podcast_player_link); ?>" style="width: 100%;">
    <?php
}
...
function cooalliance_load_more_podcasts()
{
    $paged = isset($_POST['paged']) ? intval($_POST['paged']) : 1;
...
add_action('wp_ajax_cooalliance_load_more_podcasts', 'cooalliance_load_more_podcasts');
add_action('wp_ajax_nopriv_cooalliance_load_more_podcasts', 'cooalliance_load_more_podcasts');